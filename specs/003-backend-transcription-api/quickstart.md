...
contract double also proves concurrent replay, conflict, authentication on every
operation, ownership, create-rate, active-operation, daily-usage,
cancellation/delete idempotency, queued cancellation before dispatch,
provider-independent deletion, late-result discard, non-blocking processing reads
with polling guidance, typed provider-failure recovery, expiring
create/combined-management rolling limits, canonical response shapes, terminal
replay, and cleanup-retry behavior without a network or provider call.
