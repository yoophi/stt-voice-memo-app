    ...
    pub fn link(self) {
        ...
        for package in self.packages {
            ...
            command
                ...
                // Select the target at the SwiftPM level. Xcode 27 treats
                // `--arch` as a macOS host build and appends conflicting SDK
                // and target flags after the iOS compiler overrides below.
            ...
            let legacy_search_path = out_path
                // swift build uses this output folder no matter what is the target
                .join(format!("{}-apple-macosx", arch))
                .join(configuration);

            // SwiftPM in Xcode 27 stores cross-platform products in the Xcode
            // build-system layout instead of the legacy host-triple folder.
            let xcode_configuration = if debug { "Debug" } else { "Release" };
            let xcode_search_path = out_path
                .join("out")
                .join("Products")
                .join(format!("{xcode_configuration}-{}", rust_target.sdk));
            let search_path = if xcode_search_path
